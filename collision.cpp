...
namespace IOTA_UTIL
{
	...
	bool checkPointInCircle(float cX, float cY, float cR, const MYPOINT& pt)
	{
		...
		if (radiusSquare > distSquare) return true;
	...
	bool isCollision(const MYCIRCLE& cir1, const RECT& rc)
	{
		...
		//원의 중심점이 사각형 근처에 있을 경우(가로폭 사이에 있거나, 세로폭 사이에 있거나)
			...
			(rc.top <= centerY && centerY <= rc.bottom))
		{
			...
			//사각형을 가로, 세로 4개 방향으로 반지름만큼 큰 사각형을 만들어서 원의 중점이 그 안에 있는지 비교
			if (checkPointInRect(exRect, centerX, centerY))
			{
				return true;
			}
	...
	int isCollisionOffset(const RECT& rcHold, RECT& rcMove)
	{
		...
		{
			int gapL = abs(rcHold.left - rcMove.left);
			int gapR = abs(rcHold.right - rcMove.right);

			if (gapL > gapR)
			{
				return (gapR + rcMove.right - rcMove.left);
			}
			else
			{
				return -(gapL + rcMove.right - rcMove.left);
			}
		}
