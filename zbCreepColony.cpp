...
zbCreepColony::zbCreepColony(PLAYER playerNum)
{
	_valid = true;
	...
	//유닛 고유 번호
	_buildingNumZ = BUILDINGNUM_ZERG_CREEPCOLONY;

}


zbCreepColony::~zbCreepColony()
{
}
...
void zbCreepColony::initBaseStatus(void)
{
	//Properties
	_stprintf(_baseStatus.name, L"Zerg Creep Colony");

	TCHAR strKey[100];
	_stprintf(strKey, L"ZB-creepcolonyBody%d", _playerNum);
	_baseStatus.imgBody = IMAGEMANAGER->findImage(strKey);
	_baseStatus.imgFace = NULL;
	_baseStatus.imgStat1 = IMAGEMANAGER->findImage(L"ZB-creepcolonyStat1");
...
void zbCreepColony::initBattleStatus(POINT ptTile)
{
	...
	POINT buildTileSize = BUILDSIZE_CREEPCOLONY;
...
void zbCreepColony::release(void)
{

}

void zbCreepColony::update(void)
{
	Building::update();
