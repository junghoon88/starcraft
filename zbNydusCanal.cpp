...
zbNydusCanal::zbNydusCanal(PLAYER playerNum)
{
	_valid = true;
	...
	//유닛 고유 번호
	_buildingNumZ = BUILDINGNUM_ZERG_NYDUSCANAL;

}



zbNydusCanal::~zbNydusCanal()
{
}
...
void zbNydusCanal::initBaseStatus(void)
{
	//Properties
	_stprintf(_baseStatus.name, L"Zerg Nydus Canal");

	TCHAR strKey[100];
	_stprintf(strKey, L"ZB-nyduscanalBody%d", _playerNum);
	_baseStatus.imgBody = IMAGEMANAGER->findImage(strKey);
	_baseStatus.imgFace = NULL;
	_baseStatus.imgStat1 = IMAGEMANAGER->findImage(L"ZB-nyduscanalStat1");
...
void zbNydusCanal::initBattleStatus(POINT ptTile)
{
	//BattleStatus
	...
	POINT buildTileSize = BUILDSIZE_NYDUSCANAL;
...
void zbNydusCanal::release(void)
{

}

void zbNydusCanal::update(void)
{
	Building::update();
