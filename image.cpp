...
bool image::HDCToFile(const char* FilePath, uint16_t BitsPerPixel)
{
	uint32_t Width = _imageInfo->width;
	uint32_t Height = _imageInfo->height;
	...
	char* Pixels = NULL;
	HDC MemDC = CreateCompatibleDC(_imageInfo->hMemDC);
	HBITMAP Section = CreateDIBSection(_imageInfo->hMemDC, &Info, DIB_RGB_COLORS, (void**)&Pixels, 0, 0);
	DeleteObject(SelectObject(MemDC, Section));
	BitBlt(MemDC, 0, 0, Width, Height, _imageInfo->hMemDC, 0, 0, SRCCOPY);
	DeleteDC(MemDC);
